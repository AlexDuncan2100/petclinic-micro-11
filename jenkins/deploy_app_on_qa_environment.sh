...
AWS_REGION=$AWS_REGION helm repo add stable-petclinica s3://petclinic-helm-charts-aduncan/stable/myapp/ || echo "repository name already exists"
AWS_REGION=$AWS_REGION helm repo update
helm package k8s/petclinic_chart
AWS_REGION=$AWS_REGION helm s3 push --force petclinic_chart-${BUILD_NUMBER}.tgz stable-petclinica
...
AWS_REGION=$AWS_REGION helm upgrade --install \
    petclinic-app-release stable-petclinica/petclinic_chart --version ${BUILD_NUMBER} \
